...
class Dana_transfer extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->akses['akses'] == 'Y') {
            $data = [
                ...
                "submenu_active" => "dana-transfer"
            ];
            $this->load->view('provinsi/dana_transfer/view', $data);
        } else {
    ...
    public function load()
    {
        if ($this->akses['akses'] == 'Y') {
            ...
            $result = $this->mquery->select_by('data_uraian_kegiatan_skpd', ['tahun' => $tahun, 'is_transfer' => 'Y'], "kode_rekening ASC");
            $data = [];
            $no = 0;
            $tahun_now=date('Y');
            foreach ($result as $r) {
    ...
    public function form()
    {
        ...
        if ($opsi == "add") {
            $data = [
                "skpd" => $this->mquery->select_data('data_skpd', 'id_skpd ASC'),
                "uraian" => $this->mquery->select_data('data_kode_rekening','id_uraian ASC'),
                "tahun" => $tahun
            ];
            $this->load->view('provinsi/dana_transfer/form_add', $data);
        } elseif ($opsi == "edit") {
            ...
            $this->load->view('provinsi/dana_transfer/form_edit', $data);
        } else {
